...
const sdk = new NodeSDK({
  resource: new Resource({
    [SemanticResourceAttributes.SERVICE_NAME]: 'sallie-ai-service',
...
// Get tracer
const tracer = trace.getTracer('sallie-ai-service');

// Middleware to add tracing to requests
export const tracingMiddleware = (req: Request, res: Response, next: NextFunction) => {
  const span = tracer.startSpan(`AI ${req.method} ${req.path}`, {
    attributes: {
...
// Function to trace AI operations
export const traceAIOperation = async <T>(
  model: string,
  operation: string,
  fn: () => Promise<T>
): Promise<T> => {
  const span = tracer.startSpan(`ai.${model}.${operation}`, {
    attributes: {
      'ai.model': model,
