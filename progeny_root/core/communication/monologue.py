...
# Try to import with fallback paths
try:
...
try:
    from ..prompts import PERCEPTION_SYSTEM_PROMPT, GEMINI_SYSTEM_PROMPT, INFJ_SYSTEM_PROMPT, SYNTHESIS_SYSTEM_PROMPT, MORAL_FRICTION_SYSTEM_PROMPT
except ImportError:
    try:
        from prompts import PERCEPTION_SYSTEM_PROMPT, GEMINI_SYSTEM_PROMPT, INFJ_SYSTEM_PROMPT, SYNTHESIS_SYSTEM_PROMPT, MORAL_FRICTION_SYSTEM_PROMPT
    except ImportError:
        PERCEPTION_SYSTEM_PROMPT = GEMINI_SYSTEM_PROMPT = INFJ_SYSTEM_PROMPT = SYNTHESIS_SYSTEM_PROMPT = MORAL_FRICTION_SYSTEM_PROMPT = None
...
try:
    from ..infrastructure.llm_router import get_llm_router, LLMRouter
except ImportError:
    try:
        from infrastructure.llm_router import get_llm_router, LLMRouter
    except ImportError:
...
try:
    ...
    try:
        ...
        get_synthesis_system = None
        SynthesisSystem = None
...
class MonologueSystem:
    ...
    """
    def __init__(self, limbic=None, memory=None):
        """Initialize Monologue System with comprehensive error handling."""
        try:
            self.limbic = limbic
            self.memory = memory
            self.llm_client = OllamaClient()  # Now routes through LLMRouter
            self.perception = get_perception_system() if get_perception_system else None
            self.synthesis = get_synthesis_system(limbic) if get_synthesis_system else None
            self.identity = get_identity_system() if get_identity_system else None  # Sallie's identity
            self.control = get_control_system() if get_control_system else None  # Control mechanism
